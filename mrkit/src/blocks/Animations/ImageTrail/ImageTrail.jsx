...
import { SimpleSpotlightCard } from '@/components/components';
...
export default function ImageTrail({ items = [], variant = 1 }) {
  ...
  return (
    <div className="w-full h-full relative z-[100] rounded-lg bg-transparent overflow-visible" ref={containerRef}>
      {items.map((project, i) => (
        <div className="content__img w-[300px] aspect-[1.1] rounded-[15px] absolute top-0 left-0 opacity-0 overflow-hidden [will-change:transform,filter]" key={i}>
          <div className="content__img-inner w-full h-full">
            <SimpleSpotlightCard
              title={project.title}
              image={project.image}
